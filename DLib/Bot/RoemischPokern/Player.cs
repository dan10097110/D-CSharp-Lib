...
namespace DLib.Bot.RoemischPokern
{
    public class Player
    {
        ...
        public Player(Game game)
        {
            ...
            Cards = new bool[] { true, false, false, false, false, false };
            finished = false;
        }

        public Player(Game game, int[] dices, bool[] cards, int[] table, bool finished)
        {
            this.game = game;
            Dices = dices.ToArray();
            Table = table.ToArray();
        ...
        public Action GetBestAction()
        {
            if (!finished)
            {
                ...
                var placeOver = RomanNumberExists && IsPlaceOver(RomanNumbers[0]);

                bool IsPlaceOver(int point)
                {
                    for (int i = 6; i >= 0 && (Table[i] > point || Table[i] == -1); i--)
                        if (Table[i] == -1)
                            return true;
                    return false;
                }

                if (Dices[0] + Dices[1] + Dices[2] < 6 && ((Dices[0] == 0 && Dices[1] == 0 && Dices[2] == 4) || placeOver))
                    actions.Add(new Action(this, 1));
                if (Cards[0] || Cards[5])
                    for (int dice = 0; dice < 3; dice++)
                        if (Dices[dice] > 0)
                            actions.Add(new Action(this, 0, !Cards[0], dice, 1));
                if ((Cards[1] || Cards[5]) && placeOver)
                ...
                foreach (var point in RomanNumbers)
                    if (point != 0)
                    {
                        ...
                        int GetIndex()
                        {
                            ...
                            for (; x > 0 && (Table[x - 1] == -1 || Table[x - 1] >= point); x--) ;
                            for (; x < 7 && (Table[x] == -1 || Table[x] < point); x++)
                                if (Table[x] == -1 && System.Math.Abs(averageTable[x] - point) < min)
                                {
                        ...
                        int GetIndexCard()
                        {
                            ...
                            else if (i == 0 && Table[i + 1] == -1)
                                return i + 1;
                            else if (i == 7 && Table[i - 1] == -1)
                                return i - 1;
                            else
                                return -1;
                        }
